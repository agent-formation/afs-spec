...
MUXI's observability system provides enterprise-grade logging with a two-tier architecture that separates infrastructure events from user-facing conversation events.

**Two-Tier Architecture:**
- **System Events** (`logging.system`): Infrastructure events (startup, MCP, A2A, errors) - simple stdout or file configuration
- **Conversation Events** (`logging.conversation`): User-facing events (request lifecycle, agent activity) - full multi-stream support

**Key Principles:**
- **Separation of Concerns**: System events for DevOps, conversation events for user tracking
- **Zero Storage**: MUXI captures and emits events but doesn't store them
- **Multi-Destination**: Send conversation events to multiple outputs simultaneously
...
### Minimal Setup (System Events Only)
```yaml
logging:
  system:
    level: "info"
...
```yaml
logging:
  # System events to file for DevOps
  system:
    level: "debug"
    destination: "/var/logs/system.log"

  # Conversation events to multiple destinations
  conversation:
    enabled: true
    streams:
...
| `agent.context.applied` | DEBUG | System message applied |
...
```yaml
logging:
  # System events to file for log aggregation
  system:
    level: "info"
    destination: "/var/logs/system.log"
